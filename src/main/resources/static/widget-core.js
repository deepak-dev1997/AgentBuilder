<!-- widget-core.js -->
/*  ==== Agent Widget  v1.1 (API-enabled) ====  */
(function () {
  /* 1. Read configuration */
  const cfg = window.AgentBuilder || {
    botId:  "bot",
    apiUrl: "http://localhost:8081/api/rag/chat",   // <-- change if needed
    themeColor: "#4CAF50",
    textColor:  "#ffffff",
    textFont:   "Inter",
  ...
  let conversationId = "0";           // updated after first reply
  ...
  function addMessage(txt, type="user") {
    ...
    if (type === "bot" && cfg.botAvatar) {
      wrap.innerHTML = `<img src="${cfg.botAvatar}" class="bot-avatar">
                        <div class="message-content" style="background:${cfg.themeColor};color:${cfg.textColor}">${txt}</div>`;
    } else {
  ...
  async function send() {
    ...
    try {
      ...
      const data = await res.json();      // {conversationId, message, response}
  ...
  /* Button / key handlers */
  launcher.onclick = () => widget.style.display =
